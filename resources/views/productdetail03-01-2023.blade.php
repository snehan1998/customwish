...
?>
    ...
    <div class="row">
    	...
    	<div class = "col-lg-7 product-content">
                        ...
                        <div class="fomright">
                            <form>
                                ...
                                <div class="custom-control custom-radio custom-control-inline al-lft">
                                    <input type="radio" class="custom-control-input" id="size-1" name="egg">
                                    <label class="custom-control-label" for="size-1"> <img src="{{ asset('img/egg.png')}}" class="egg">Egg</label>
                                </div>
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input type="radio" class="custom-control-input" id="size-2" name="eggless">
                ...
                <div class="col-lg-12">
                    ...
                    ?>
                        ...
                        <label class="LargeOptionRadio__label">
                            <input class="form-control sradioatt sradio{{$attvalue->id}} product-filter-item sradioo{{$attvalue->attr_id}}" value="{{$attvalue->id}}" name="attsname{{$attvalue->attr_id}}" type="radio" id="sradioatt" data-at="{{$attvalue->attr_id}}" data-attr="{{$attvalue->var_id}}"  onclick="getattr(); getclass(); getimages();">
                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <p class="pflex">
                                        ...
                                        <option value="{{$cahr->id}}"  onclick="getattr(); getclass(); getimages();">{{$cahr->attr_value_title}}</option>
                                        @endforeach
                                        </select>
                                    </p>
                                </div>
            ...
            @endif-->
            <button type="button" class="logbtn add-to-varcartt1" style="width: 100%; ">ADD TO CART</button>
        ...
        $('.add-to-varcartt1<?php echo $product->id; ?>').click(function(e){
            ...
            //alert(variation_id);
            var slougon = $('.slougon<?php echo $product->id; ?>').val();
            var sradioo = $('.sradioo<?php echo $attvalue1->attr_id; ?>:checked').val();
            ...
            $.ajax({
                ...
                method: "POST",
                data: {_token: '{{ csrf_token() }}',"price": price,"sradioo":sradioo,"colorradioo":colorradioo,"session_id":session_id,"product_id":product_id,"quantity":quantity,"variation_id":variation_id,"slou":slougon},
                dataType: "json",
                success: function (response) {
...
function getattr() {
    ...
    var data = $.parseJSON($.ajax({
                url: "{{url('/loadd')}}",
                type: "POST",
                data: {'product':productdd, 'att': a, _token: '{{csrf_token()}}' },
                dataType: 'JSON',
                async: false
    }).responseText);
