    	...
    	<div class = "col-lg-7 product-content">
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    ...
                    <p style="display: flex;"> <input type="file" name="imageupload[]" multiple id="imageuploadd" class="imageupload{{$product->id}}" style="padding-left:10px;" accept="image/*"></p>
                </div>
                @endif
                @if($product->uploadlogo_option == 1)
                <div class="col-lg-6">
                    <h5>{{$product->uploadlogo_heading}}</h5>
                    <p style="display: flex;"> <input type="file" name="logoupload[]" multiple id="logoupload" class="logoupload{{$product->id}}" style="padding-left:10px;" accept="image/*"></p>
                </div>
                @endif
            </div>
            <div class="row">
...
?>
    ...
    <div class = "col-lg-7 product-content">
        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                <h5>{{$product->uploadlogo_heading}}</h5>
                <p style="display: flex;"> <input type="file" name="logoupload[]" multiple id="logouploadd" class="logoupload{{$product->id}}" style="padding-left:10px;" accept="image/*"></p>
            </div>
            @endif
        </div>
        <div class="row">
...
$(document).ready(function() {
  if (window.File && window.FileList && window.FileReader) {
    $("#imageuploadd").on("change", function(e) {
      ...
      for (var i = 0; i < filesLength; i++) {
        ...
        fileReader.onload = (function(e) {
          var file = e.target;
          $("<span class=\"pip\">" +
            "<img style=\"width:25%;height:67%;\" class=\"imageThumb\" src=\"" + e.target.result + "\" title=\"" + file.name + "\"/>" +
            "<br/><span class=\"remove\">Remove image</span>" +
            "</span>").insertAfter("#imageuploadd");
...
<script>
    $(document).ready(function() {
      if (window.File && window.FileList && window.FileReader) {
        $("#logouploadd").on("change", function(e) {
          var files = e.target.files,
            filesLength = files.length;
          for (var i = 0; i < filesLength; i++) {
            var f = files[i]
