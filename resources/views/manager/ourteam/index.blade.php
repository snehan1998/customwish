        ...
        <div class="row">
            <div class="col-12">
                    <div class="card">
                        ...
                        <a href="{{ route('manager.ourteam.create') }}" class="btn btn-primary btn-sm scroll-click" >
                            create</a>
                        </div>
                        </div>
                                ...
                                <table id="example" class="display" style="min-width: 845px">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>{{$i++}}</td>
                                            <td>{{ @$row->name }}</td>
                                            <td width="250px"><img src="{{ asset('uploads/images/') }}/{{ @$row->team_image }}" width="20%" /></td>
                                            <td><a href="{{ route('manager.ourteam.edit', $row->id) }}" class="btn btn-primary">Edit</a>
                                            <button form="resource-delete-{{ $row->id }}" class="btn btn-danger btn-icon-style-2"><span>Delete</span></button>
                                                <form id="resource-delete-{{ $row->id }}" action="{{ route('manager.ourteam.destroy', $row->id) }}" style="display: inline-block;" onSubmit="return confirm('Are you sure you want to delete this item?');" method="post">
                                                @csrf
                                                @method('DELETE')
                                                </form>
                                            </td>
