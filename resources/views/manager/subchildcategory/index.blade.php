                        ...
                        <div class="card">
                            <div class="card-header">
                                ...
                                <div class="text-right">
                                    <a href="{{ url('/manager/subchildcategory/create') }}" class="btn btn-primary btn-sm scroll-click"> create</a>
                                </div>
                            </div>
                            <div class="card-body">
                            <div class="table-responsive">
                                    <table id="example" class="display" style="min-width: 845px">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{$i++}}</td>
                                                <td  width="250px"><img src="{{ asset('uploads/images/') }}/{{ @$row->subchildcat_logo }}" style="width: 30%;" /></td>
                                                <td>{{ @$row->category->cat_name }}</td>
                                                <td>{{ @$row->subcategory->subcat_name }}</td>
                                                <td>{{ @$row->childcategory->childcat_name }}</td>
                                                <td>{{ @$row->subchildcat_name }}</td>
                                                <td>{{ @$row->subchildcat_slug }}</td>
                                                <td><span class="badge badge-primary">{{ @$row->subchildstatus }}</span></td>

                                                <td class="color-primary"><a href="{{ route('manager.subchildcategory.edit', $row->id) }}" class="btn btn-primary"><i class="fa fa fa-pencil m-0"></i></a>
                                                ...
                                                <form id="resource-delete-{{ $row->id }}"
                                                    action="{{ route('manager.subchildcategory.destroy', $row->id) }}"
