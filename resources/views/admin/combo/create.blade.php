            ...
            <!-----------------------------Add Variations--------------------------------------------------->
                <div class="card-body">
                    <div class="form-validation">
                          <form enctype="multipart/form-data"  method="post" action="{{ route('admin.combo.store') }}">
                            @csrf
                                    ...
                                    <table id="example" class="display" style="min-width: 845px">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('admin.combo.edit', $row->id) }}" class="btn btn-primary"><i class="fa fa fa-pencil m-0"></i></a>
                                                    <button form="resource-delete-{{ $row->id }}" class="btn btn-danger btn-icon-style-2"><span><i class="fa fa-trash"></i></span></button>
                                                    <form id="resource-delete-{{ $row->id }}" action="{{ route('admin.combo.destroy', $row->id) }}" style="display: inline-block;" onSubmit="return confirm('Are you sure you want to delete this item?');" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    </form>
                                                    @if($row->is_combo	== '1')
                                                    <a  href="{{ url('/') }}/admin/combo/addcombo/{{ $row->id }}" class="btn btn-primary">Add Combo</a>
                                                    @endif
                                                </td>
                                            </tr>
                                            @endforeach
