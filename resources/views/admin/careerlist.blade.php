                            ...
                            <div class="table-responsive">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ @$row->position }}</td>
                                        <td>{{ @$row->message }}</td>
                                        <td><a href="http://127.0.0.1:8000/public/uploads/pdf/{{$row->resume}}" download><i class="fa fa-download"></i></a></td>
                                        <td><button form="resource-delete-{{ $row->id }}" class="btn btn-danger btn-icon-style-2"><span>Delete</span></button>
                                            <form id="resource-delete-{{ $row->id }}" action="{{url('/admin/careerdestroy')}}/{{$row->id}}" style="display: inline-block;" onSubmit="return confirm('Are you sure you want to delete this item?');" method="post">
                                            @csrf
                                            @method('DELETE')
                                            </form>
                                        </td>
