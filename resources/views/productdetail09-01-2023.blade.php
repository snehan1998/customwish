    ...
    <div class="row">
    	...
    	<div class = "col-lg-7 product-content">
                ...
                <div class="col-lg-12">
                    ...
                    ?>
                       ...
                       <!--<label class="boxed">
                        ...
                        // dd($variant);
                        ?>
                        @if($attnamee1show->attr_label == 'button')
                        <div class="row">
                            <div class="col-lg-12">
                                @foreach($variantt1show as $variantt1show)
                                @if($variantt1show->main_attr_id == $attribute->product_attr_id)
                                <?php   $attvaluee1show = App\Models\AttributeValue::where('id',$variantt1show->main_attr_value)->first();
                                       $fetchvariantt1show = App\Models\AddSubVariation::where('id',$variantt1show->var_id)->first();
                                       $producimgss1show = App\Models\ProductImage::where('product_id',$fetchvariantt1show->product_id)->where('variation_product_id',$fetchvariantt1show->id)->first();
                               ?>
                                <p class="gst">{{$attvaluee1show->attr_value_title}}<button type="button" data-aid="{{$attvaluee1show->id}}" id="onshoww" value="{{$attvaluee1show->id}}" onclick="opencolor();">{{$attnamee1show->attr_name}}</button></p>
                                @endif
                                @endforeach
                            </div>
                        </div>
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    <p class="pflex">
                        <select class="form-select inputclass12 charm_id{{$product->id}}" aria-label="Default select example" name="charm_id" style="color: gray;">
                        <option value="" selected disabled>{{$optionheading->product_select_title}}</option>
        ...
        $('.add-to-varcartt1<?php echo $product->id; ?>').click(function(e){
            ...
            var giftwrap = $('.giftwrap<?php echo $product->id; ?>').val();
            ...
            $.ajax({
                ...
                method: "POST",
                data: {_token: '{{ csrf_token() }}',"price": price,"sradioo":sradioo,"colorradioo":colorradioo,"sselect":sselect,"session_id":session_id,"product_id":product_id,"quantity":quantity,"variation_id":variation_id,
                "comment":comment,"description":description,"giftwrap_price":giftwrap_price,"giftwrap":giftwrap,"pickup_type":pickup_type,"timee":timee,"datee":datee,"flowerss_type":flowerss_type,"location":location,
                "colortype":colortype,"printside":printside,"charm_id":charm_id,"addtext2":addtext2,"addtext1":addtext1,"logoupload":logoupload,"imageupload":imageupload,"quantity":quantity,"eggtype":eggtype,"a":a},
                dataType: "json",
                success: function (response) {
    ...
    <div class = "col-lg-7 product-content">
        ...
        <div class="row">
            ...
            <div class="col-lg-6">
                ...
                ay: flex;"> <input type="file" name="imageupload" id="imageuploadd" class="imageupload{{$product->id}}" style="padding-left:10px;" accept="image/*"></p>
            </div>
            @endif
            @if($product->uploadlogo_option == 1)
            <div class="col-lg-6">
...
var imageupload = document.getElementById("imageuploadd").files[0].name;
alert(imageupload);
   ...
   $.ajax({
        ...
        data: {_token: '{{ csrf_token() }}',"price": price,"session_id":session_id,"product_id":product_id,"quantity":quantity,
            "comment":comment,"description":description,"giftwrap_price":giftwrap_price,"giftwrap":giftwrap,"pickup_type":pickup_type,"timee":timee,"datee":datee,"flowerss_type":flowerss_type,"location":location,
            "colortype":colortype,"printside":printside,"charm_id":charm_id,"addtext2":addtext2,"addtext1":addtext1,"logoupload":logoupload,"imageupload":imageupload,"quantity":quantity,"eggtype":eggtype},
        dataType: "json",
        success: function (response) {
...
function getattr() {
 ...
 /*       if(v != "" || v != undefined || v != null)
        ...
        var data = $.parseJSON($.ajax({
            url: "{{url('/loadd')}}",
            type: "POST",
            data: {'product':productdd, 'att': a, 'selecct':v , _token: '{{csrf_token()}}' },
            dataType: 'JSON',
            async: false
    }).responseText);
