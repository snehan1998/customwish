...
<div class="row">
    	...
    	<div class = "col-lg-7 product-content">
            ...
            <?php $productcombo1 = App\Models\ProductCombo::where('product_id',$product->id)->get();?>
            @foreach($productcombo1 as $productcombo1)
            <h2 class="deco">Input Personalization Details</h2>
            <div class="row">
            <div class="col-lg-6 ">
                <p style="display: flex;">For {{$productcombo1->button_name}}{{$productcombo1->id}}:
                <button class="logbtn1" value="{{$productcombo1->id}}" type="button" data-id="{{$productcombo1->id}}" data-box="buttonattr{{$productcombo1->id}}">Show</button></p>
            </div>
            <div class="showonlyclick{{$productcombo1->id}}" style="display: none" id="buttonattr{{$productcombo1->id}}">
                @if($productcombo1->combo_text_field == 1)
                <div class="row">
                    <div class="col-lg-12">
                        <h5 class="song">{{$productcombo1->combo_text_heading}}<span class="char">@if($productcombo1->combo_text_validation != "")(Max {{$productcombo1->combo_text_validation}} characters ) @endif</span></h5><br>
                        <p style="display: flex;"><input type="text" class="inputclass11 comboaddtext2{{$productcombo1->id}}" name="addtext2" maxlength="{{$productcombo1->combo_text_validation}}" placeholder="Type here" style="margin:0px 0px 0px 10px;"></p>
                    </div>
                </div>
                @endif
                @if($productcombo1->is_charm == 1)
                <div class="row">
                    <?php  $optionheading = App\Models\ProductSelectHeading::where('product_id',$product->id)->where('combo_id',$productcombo1->id)->get();?>
                    @foreach($optionheading as $optionheading)
                    <?php $option = App\Models\ProductSelectOption::where('product_id',$product->id)->where('combo_id',$productcombo1->id)->where('product_select_id',$optionheading->id)->get(); ?>
                    <div class="col-lg-12">
                        <p class="pflex">
                            <select class="form-select inputclass12 charm_id{{$product->id}}" onclick="getcharmid();" id="charm_idd" aria-label="Default select example" name="charm_id" style="color: gray;">
                            <option>{{$optionheading->product_select_title}}</option>
                ...
                @endif
                <?php
                    $comboattribute = App\Models\ProductComboVariation::where('product_id', $product->id)->get();
                ?>
                @foreach($comboattribute as $comboattribute)
                <?php $comboattname = App\Models\Attribute::where('id',$comboattribute->product_combo_attr_id)->first();
                $combovariantt = App\Models\ProductCombooo::where('product_id',$product->id)->where('combo_attr_id',$comboattribute->product_combo_attr_id)->get();
                $combovariant = $combovariantt->unique('combo_attr_value','id');
          ?>
              @if($comboattname->attr_label == 'label')
              <label class="slect-varient">Select Variant</label>
              <div class="boxed">
              @foreach($combovariant as $combovariant)
              @if($combovariant->combo_attr_id == $comboattribute->product_combo_attr_id)
              <?php
                  $comboattvalue = App\Models\AttributeValue::where('id',$combovariant->combo_attr_value)->first();
                  $combofetchvariant = App\Models\ProductCombo::where('id',$combovariant->combo_id)->first();
              ?>
              <label class="LargeOptionRadio__label">
                  <input class="form-control sradioatt sradio{{$comboattvalue->id}} product-filter-item sradioo{{$product->id}}" value="{{$comboattvalue->id}}" name="attsname{{$comboattvalue->attr_id}}" type="radio" id="sradioatt" data-at="{{$combofetchvariant->combo_id}}" data-attr="{{$combofetchvariant->combo_id}}"  onclick="getattr(); getclass(); getimages();">
                  <span class="sradio" data-name="{{$comboattname->attr_name}}">
                  {{$comboattvalue->attr_value_title}}
              </label>
              @endif
              @endforeach
              </div>
              @endif
              <?php
                    $comboattnamee1 = App\Models\Attribute::where('id',$comboattribute->product_combo_attr_id)->first();
                    $combovarianttt1 = App\Models\ProductCombooo::where('product_id',$product->id)->where('combo_attr_id',$comboattribute->product_combo_attr_id)->get();
                    $combovariantt1 = $combovarianttt1->unique('combo_attr_value');
                    $combofetchvarianttext1 = App\Models\ProductCombo::where('product_id',$product->id)->where('combo_attr_id',$comboattribute->product_combo_attr_id)->first();
                ?>
                @if($comboattnamee1->attr_label == 'color')
                <div class="row">
                    <div class="col-lg-12">
                        <p class="gst">Color </p>
                        <div class="preview">
                            @foreach($combovariantt1 as $combovariantt1)
                             @if($combovariantt1->combo_attr_id == $comboattribute->combo_attr_id)
                             <?php  $comboattvaluee1 = App\Models\AttributeValue::where('id',$combovariantt1->combo_attr_value)->first();
                                    $combofetchvariantt1 = App\Models\ProductCombo::where('id',$combovariantt1->combo_id)->first();
                                ?>
                                <label><!--name="attr_{{$attnamee1->attr_name}}"-->{{$attnamee1->attr_name}}
                                <input type="hidden" name="product_id" value="{{$product->id}}"/>
                                <input style="background-color:{{$comboattvaluee1->attr_value_title}};" class="form-control cradio{{$comboattvaluee1->id}} product-filter-item colorradioo{{$product->id}}" value="{{$comboattvaluee1->id}}"  name="attname" type="radio" id="colorradio"  data-attr="{{$combovariantt1->combo_id}}"  onclick="getattr(); getclass(); getimages();">
                                <span  class="colorradio" data-name="{{$comboattnamee1->attr_name}}" data-bg-color="#{{$comboattnamee1->attr_value_title}}" class="size-code">
                                </label>
                                @endif
                            @endforeach
                        </div>
